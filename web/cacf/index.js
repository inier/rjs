	define([
        ...
        '../rock/js/qrCode/jquery.qrcode.min.js',
        'css!../recommend/recommend.scss',
        'css!../bounce/bounce.css',
        'css!../rock/com/jquery-pagination/css/pagination.css'
    ],
    function ($, BaseClient, RockBase, Tpl, Pop, Dialog, Cart, GoodsManager, Client, Recommend, Feedback, lazyload, Paging, DataSource) {
        ...
        function userLuckyNumDialog() {
            ...
            function init(ev, data) {
                ...
                var tpl = _.template($('#user-detail-template').html())({
                    data: data
                });
                            ...
                            (function (stock) {
                                ...
                                if (none_skus && none_skus.length > 0) {
                                    for (var n = 0; n < none_skus.length; n++) {
                                        _.each(_.keys(none_skus[n]), function (k) {
                                            if (k != "price" && k != "goodsId") {
                                                var d = {};
                                                var v = none_skus[n][k];
                                                d[k] = v;
                                                if (_.where(none_skus, d).length == _.where(myData, d).length) {
                                                    $(document.body).find("[data-key='" + k + "']").each(function (index, el) {
        ...
        function showActivityNotStart(data) {
            ...
            $('.activity .all-btn a').addClass('disabled').removeClass('buy-now');
            $('.activity').show().removeClass('hide');
            ...
            //显示时间
            el.find('.time-describe').text('开始时间：');
        ...
        function showActivityNow(data) {
            ...
            el.find('.progress-bar').css('width', percent * 100 + '%');
            el.find('.progress-speed').text(showPercent + '%');
            //距离结束时间倒计时
            el.find('.time-describe').text('距离结束时间：');
            countTime('now');
        ...
        function showActivityWaitOpen(data) {
            ...
            $('.activity').hide();
            $('.activity-writing').show().removeClass('hide');
            $('.activity-die').hide();
            $('.unable-buy').hide();
            $('.lucky-user').hide();
            ...
            if (data.state == '6') {
                ...
                messageInfoDom.find('img').remove();
                messageInfoDom.find('.info').text('中奖号码生成中，请稍等。');
            } else {
                //显示揭晓时间
                countTime('waitOpen');
                //中奖规则显示
        ...
        function showActivityHasOpen(data) {
            ...
            if (data.user.nickName)$(info[0]).text(data.user.nickName);
            if (data.openTime)$(info[1]).text(data.openTime);
            if (data.user.partCopies)$(info[2]).text(data.user.partCopies + '人次');
            if (data.user.buyTime)$(info[3]).text(data.user.buyTime);
        ...
        function showActivityDie(data) {
            ...
            $('.activity').hide();
            $('.activity-writing').hide();
            $('.activity-die').show().removeClass('hide');
            $('.unable-buy').hide();
            ...
            el.find('.progress-speed').text(showPercent + '%');
            //距离结束时间倒计时
            el.find('.time-describe').text('距离结束：');
            el.find('.leave-time').text('已过期');

            el.find('.die-tip span').text(data.productName);
        ...
        function showActivityUnableBuy(){
            ...
            $('.activity-writing').hide();
            $('.activity-die').hide();
            $('.unable-buy').show().removeClass('hide');
        ...
        function getSetting(data) {
            ...
            if (data) {
                _.each(allKeys, function (key) {
                    if (key != "GoodsId") {
                        $(document.body).find("[data-key='" + key + "']").each(function (i, el) {
                            if ($(el).hasClass('option-active')) {
        ...
        return RockBase.View.extend({
            ...
            onShow: function () {
                ...
                window.onmousewheel = document.onmousewheel = me.scrollFunc.bind(me);//IE/Opera/Chrome/Safari
                //拖动滚动条事件
            ...
            doInit: function () {
                ...
                $('.parts-show').on("click", ".choose-img-little", function (event) {
                    ...
                    $(".detail-img").find("img").attr("src", img);
                    return false;
                });
                ...
                $('.parts-show').on("click", ".last", function () {
                    ...
                    animateState = 1;
                    $(".carousel-content").animate({marginLeft: newLeft}, function () {
                        animateState = 0;
                    });
                    if (hide) {
                        $('.last').css('visibility', 'hidden');
                ...
                //点击回到顶部
                $("#js-siteGoTop").on("click", function () {
                    $('html,body').animate({
...
//                        var goodsId = me.goodsId[0];
                ...
                if ($('#car #goodsStatus').val() == '0') {
                    ...
                    $('.confirm-order-header').unbind('click').bind("click", buyNow);
                }
                ...
                function barChange(e) {
                    ...
                    var goHere = $(el).attr("class").substr(6);
                    $(el).addClass("nav-active");

                    //使相应区域的元素显示在页面上
                    var offset = $("." + goHere).offset().top;
                    // document.documentElement.scrollTop = offset - 55;
                    // document.body.scrollTop = offset - 55;
//                    if ($("body").scrollTop()) {
                    $("html,body").animate({scrollTop: offset - 55}, 1000);
//                    }
                    return false;
                }
            ...
            //判断特定元素在平面中的位置，做相应的处理
            //触发的事件的鼠标滚轮滑动和拖动页面右侧的滚动条，进入页面的时候
            scrollFunc: function () {
                var me = this;
                ...
                var dTop = $(".detail").offset().top;
                // var pTop = $(".param").offset().top;
                var eTop = $(".evaluation").offset().top;
                ...
                //获取详情图片的div距离浏览器顶部的距离
                //获取详情图片的div距离页面顶部的距离
                var theDistance = dTop - sTop;
//            console.log(theDistance);
                //根据div相对于浏览的顶部的位置，通过改变class实现div的状态改变
...
//                console.log(dtheDistance);
                ...
                if (reviewAjaxFlag == 0) {
                    ...
                    var theReviewTop = $(".evaluation").offset().top;
...
//                            console.log("用户的浏览数据提交");
            ...
            _initParticipationRecord: function () {
                ...
                var dataRender = function (items, data) {
                    ...
                    if (items && items.length && items.length != 0) {
                        _.each(items, function (item) {
                            var img;
                            item.img ? img = BaseClient.basePath + 'goods-img/img' + item.img : img = '../rock/img/default-head.png';
                ...
                var paging = new Paging({
                    ...
                    beforeChange: function () {
                        var offset = $('.evaluation').offset().top;
                        $("html,body").animate({scrollTop: offset - 70}, 0);
                    }
                });
            },
            ...
            createQRCode: function () {
                ...
                if (isCanvas) {
                    $('#code').qrcode({
                        ...
                        width: 120,
                        height: 120,
                        text: "http://cloud.mall.changan.com.cn/caecapp/main/index.html#classification/goods-detail-crowdfunding.html?goodsid=" + productId + "&isShare=true"
                    });
                } else {
                    $("#code").qrcode({
                        render: "table",
            ...
            changeImgShow: function (key) {
                ...
                if (me.optionImg[key] && me.optionImg[key].length > 0) {
                    ...
                    detailContent.find('.choose-img-little').removeClass('show-active');
                    detailContent.find('.detail-img-content img').attr('src', theCompleteSrc);
                }
            },
