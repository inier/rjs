...
require([
    ...
    'flexslider',
    'rock/js/nav-main'
], function ($, lazyload, domReady, CartMenu, BaseClient, User, Paging, DataSource, Recommend, placeholder, Raty, Checkbox, Feedback, GoBack, _) {
    ...
    function CarDiy() {
        ...
        var move = function (tmpX) {
            abs = tmpX + ctrlPaletteWidth - OFFSET;
            ...
            _.each(IMAGES, function (data) {
                if (data && data.url) {
                    ...
                    $(newImg).error(function(){
                        alert("图片加载失败，请尝试刷新浏览器！");
                    });
                    oImgs.push(newImg);
                    tDom += '<img src="'+data.url+'" class="'+data.class+'" alt=""/>';                    
                    if (oImgs.length > imgNum) {
                        if (loadingEl.hasClass("show")) {
                            loadingEl.removeClass("show");
                        }
                        $('#carBuilderWrap').append(tDom);
                        $(config.elWrap).removeClass("hides");
                        bindEvent();
                    }
                }
            });
    ...
    CarDiy();
    // 返回顶部-预约试驾等悬浮板块 在线客服跳转处理(暂未引入live800Url.js) live800Url.init({
    // id:"#js-live800Side" }); 意见反馈
